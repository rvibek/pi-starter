...
- `context7` in `.pi/mcp.json` must NOT contain `"transport"` or `"lifecycle"` fields — they are non-standard and not read by Pi. Use `"command"`/`"args"` (stdio is implied) or `"type": "streamable-http"` for remote servers.
